...
def montar_etiqueta(pedido: dict, fallback_addr: dict = None) -> Optional[dict]:
    """Monta etiqueta de transporte no formato esperado pelo Bling."""
    addr = pedido.get("shipping_address") or pedido.get("billing_address") or fallback_addr
    if not addr:
        return None
    nome = (
...
async def criar_pedido_bling(
    ...
    data_pedido = pedido.get("created_at", "")[:10]  # YYYY-MM-DD
    ...
    # Transporte com etiqueta de entrega
    ...
    async def executar() -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30) as client:
                ...
                # Busca IDs dos produtos no Bling
                yield log("info", "🔎 Localizando produtos no Bling pelo SKU...")
                for item in pedido.get("line_items", []):
                    yield log("info", f"   → {item.get('quantity')}x {item.get('title')} (SKU: {item.get('sku', '?')}) — R$ {item.get('price')}")
